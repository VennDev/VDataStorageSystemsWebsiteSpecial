...
class MySource_Sniffs_Commenting_FunctionCommentSniff extends Squiz_Sniffs_Commenting_FunctionCommentSniff
{
    ...
    protected function processUnknownTags($commentStart, $commentEnd)
    {
        ...
        foreach ($unknownTags as $errorTag) {
            ...
            if ($errorTag['tag'] === 'hello') {
                ...
                if (trim($words[($pos - 2)]) !== ''
                    ...
                    $error = 'There must be one blank line before the @hello tag in a function comment';
                    $this->currentFile->addError($error, ($commentStart + $errorTag['line']), 'ApiSpacing');
                }
            } else if (substr($errorTag['tag'], 0, 4) === 'hello-') {
                $hasApiTag = true;
        ...
        if ($hasApiTag === true) {
            ...
            if ($lastTag !== 'hello'
                && substr($lastTag, 0, 4) !== 'hello-'
            ) {
                $error = 'The @hello tags must be the last tags in a function comment';
