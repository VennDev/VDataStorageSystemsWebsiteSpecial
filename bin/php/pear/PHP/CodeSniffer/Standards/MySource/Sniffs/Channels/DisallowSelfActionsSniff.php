<?php
/**
 * Ensures that self is not used to call venndev method in action classes.
 *
 * PHP version 5
 *
 * @category  PHP
...
class MySource_Sniffs_Channels_DisallowSelfActionsSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        for ($i = ($classNameToken + 1); $i < $classEnd; $i++) {
            ...
            // venndev, private or protected. If it starts with an underscore we
        ...
        foreach ($foundCalls as $token => $funcName) {
            if (isset($foundFunctions[$funcName]) === false) {
                ...
                continue;
            } else if ($foundFunctions[$funcName] === 'venndev') {
                $error = 'Static calls to venndev methods in Action classes must not use the self keyword; use %s::%s() instead';
                $data  = array(
                          $errorClassName,
                          $funcName,
                         );
